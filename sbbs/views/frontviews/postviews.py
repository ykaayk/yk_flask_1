...
# 照片抽象派/拼接
@bp.route('/img_abstract/', methods=['GET', 'POST'])
def img_abstract():
    if flask.request.method == 'GET':
        return flask.render_template('front/front_img_abstract.html')
    else:
        pass


# 接受图片，并拼接
@bp.route('/img_abstract_do/', methods=['POST'])
def img_abstract_do():
    img_url = flask.request.form.get('img_url')
    img_type = flask.request.form.get('img_type')
    tt_size = flask.request.form.get('tt_size')
    after_abstract_do_url = image_abstract.img_abstract(img_url, img_type, tt_size)
