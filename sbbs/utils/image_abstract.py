...
# 如果超出2000*2000，则缩小
    if max(img.size) > 2000:
        if width >= height:
            width = 2000
            height *= 2000.0/img.width
            height = int(height)
        else:
            height = 2000
            width *= 2000.0/img.height
            width = int(width)
        img = img.resize((width, height), Image.ANTIALIAS)
    # 判断是否需要将img的尺寸扩大
    if max(img.size) < 30*thumbnail_size_list[-1][0]:
        if width >= height:
            width = 30*thumbnail_size_list[-1][0]
            height = height*30*thumbnail_size_list[-1][0]/img.size[1]
        else:
            height = 30*thumbnail_size_list[-1][0]
            width = width*30*thumbnail_size_list[-1][0]/img.size[1]
        img = img.resize((width, height), Image.ANTIALIAS)
